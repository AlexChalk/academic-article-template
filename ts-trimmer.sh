...
tst() {
  ...
  titles_oneline=$(echo "${short_titles[@]}" | sed --expression :a --expression '/\]$/!N; s/\n/\ /; ta')

  titles_clean=$(echo "${titles_oneline[@]}" | tr --delete '^[' | tr --delete ']$')

  while read -r title; do
    local count
    local regexp_title

    regexp_title="${title// /[[:space:]]}"

    count=$(echo "$article" | rg --multiline --count "$regexp_title")
